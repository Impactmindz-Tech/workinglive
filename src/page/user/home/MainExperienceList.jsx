...
const MainExperienceList = ({ product }) => {
  ...
  const golive = async (data) => {
    ...
    try {
      ...
      if (res?.isSuccess) {
        const isWithinSlot = res.remainingSlots.some((slot) => currentTime >= slot.from && currentTime <= slot.to);
        if (isWithinSlot) {
  ...
  return (
    <>
      ...
      <div className="max-w-sm rounded overflow-hidden shadow-lg sm:max-w-full h-full relative">
        <Link to={`/book-experience/${product?._id}`}>
          {" "}
          <SwiperSlider setheight={true} item={product.images || Images.cardRoundedEqual} thumnail={product?.thumbnail} />
        </Link>
        <div className="px-5 py-4 sm:px-3">
          <div className="font-bold text-xl mb-2 relative first-letter:capitalize">
            <Link to={`/book-experience/${product._id}`}>{product.ExperienceName}</Link>
            <div className="absolute -top-12 right-0 z-40">
              <img src={product.avatarImage || Images.user2} alt="user" className="w-[60px] h-[60px] sm:w-14 sm:h-14 rounded-full border object-cover border-white shadow-md" />
            </div>
          </div>
          ...
          <div className="flex gap-2 items-center justify-between">
            <p className="text-gray-700 text-base w-[60%] lg:w-[58%] sm:text-xs font-medium">Starts at: ${product.AmountsperMinute} (Per Minute)</p>
            {product?.bookinstaltly && (
              <Link to="/auth/login">
                <button className="flex items-center px-4 py-1 bg-gradient-to-r from-red-500 to-pink-500 text-[10px] text-white font-semibold rounded-md shadow-md">
