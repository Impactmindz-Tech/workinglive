    ...
    if (socket) {
      ...
      const handleIncomingMessage = (data) => {
        if (data.receiverId === user?._id) {
          toast(`New message from ${data.user.fullname}: ${data.message}`, { duration: 6000 });
        }
        setMessages((prevMessages) => [...prevMessages, { user: data.user, message: data.message, time: data.time }]);
      };
              ...
              {messages.map(({ message, user: { id } = {}, time }, index) => {
                ...
                return (
                  <div key={index} className={`pt-7 ${isCurrentUser ? "text-right" : "text-left"}`}>
                    <div className={`max-w-[40%] p-3 ${isCurrentUser ? "bg-primary text-white rounded-bl-lg rounded-t-lg ml-auto" : "bg-secondary rounded-t-lg rounded-br-lg"}`}>
                      {typeof message?.message === "string" ? message.message : ""}
                      {typeof message === "string" ? message : ""}
                    </div>
                    <p className="text-grey-800 mt-1 text-xs">{time ? formatTime(new Date(time)) : formatTime(new Date())}</p>
                  </div>
                );
              })}
            </div>
