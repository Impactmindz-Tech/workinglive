...
const Login = () => {
  ...
  const handleAppleSignin = async (e) => {
    // appleProvider.addScope("Tinku1252@gmail.com");
  ...
  const handleEmailSubmit = async (e) => {
    ...
    const actionCodeSettings = {
      url: "http://localhost:5173/auth/login", // The URL where users will be redirected after clicking the sign-in link
    ...
    const handleSignInWithEmailLink = async () => {
      if (isSignInWithEmailLink(auth, window.location.href)) {
        ...
        try {
          ...
          const response = googlesignupandsigninApi(body);
          if (response?.isSuccess) {
            alert("successfull");
          }
          // navigate("/user/dashboard");
