  ...
  const handleOtherFileChange = (e) => {
    if (e.target.files) {
      ...
      const validFiles = files.filter((file) => {
        if (!file.type.startsWith("image/")) {
          toast.error(`Invalid file detected: ${file.name}. Please select only image files.`);
          return false;
        }
        return true;
      });
      ...
      if (validFiles.length > 0) {
        ...
        setOtherImageURLs((prevURLs) => [...prevURLs, ...validFiles.map((file) => URL.createObjectURL(file))]);
      }
    }
  };
      ...
      <div className="px-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <div className="flex justify-between my-4 flex-wrap">
            <div className="w-[49%] relative">
              <div className="absolute top-2 right-2 flex gap-2">
                ...
                {selectedFile && (
                  <div className="cursor-pointer bg-white p-4 sm:p-2 rounded-md BoxShadowLessRounded" onClick={handleRemoveMainImage}>
                    <img src={Images.close} alt="remove" className="cursor-pointer w-6 h-6" />
                  </div>
                )}
              </div>

              {!imageURL ? (
                <div onClick={handleMainImageClick} className="border rounded-lg w-full  h-[240px] sm:h-[140px] bg-[#f2f2f2] border-[#e2e2e2] flex justify-center items-center flex-col cursor-pointer group py-5">
                  ...
                  <div className="flex justify-center p-2 bg-white rounded-md">
                    <img src={Images.add} alt="add" className="w-8 h-8 object-contain" />
                  </div>
                  <h1 className="text-center text-grey-800 pt-2 font-semibold group-hover:text-grey-900 sm:text-base">Main Image</h1>
                </div>
              ) : (
                <img src={imageURL} alt="Selected" className="w-full object-cover rounded-2xl z-10 h-[240px] sm:h-[140px]" />
              )}
            </div>

            <div className="w-[49%] h-full relative">
              <div onClick={handleOtherImageClick} className="border  h-[240px] sm:h-[140px] rounded-lg w-full bg-[#f2f2f2] border-[#e2e2e2] flex justify-center items-center flex-col group cursor-pointer py-5">
                <input className="hidden" onChange={handleOtherFileChange} ref={otherImage} type="file" multiple accept="image/*" />
                <div className="flex justify-center p-2 bg-white rounded-md">
                  <img src={Images.add} alt="add" className="w-8 h-8 object-contain" />
                </div>
                <h1 className="text-center text-grey-800 pt-2 font-semibold group-hover:text-grey-900 sm:text-base">Other Images</h1>
              </div>
            </div>
          </div>
          ...
          <div className="my-2">
            ...
            <input type="text" name="ExperienceName" id="exp-name" className="input my-2" placeholder="Eiffel Tower" {...register("ExperienceName")} />
          ...
          <div className="my-2">
            ...
            <input type="number" name="AmountsperMinute" id="AmountsperMinute" className="input my-2" placeholder="$5" {...register("AmountsperMinute")} />
          ...
          <div className="my-2">
            ...
            <textarea name="notesForUser" rows={5} id="notesForUser" className="input my-2 resize-none" placeholder="Type notes" {...register("notesForUser")}></textarea>
