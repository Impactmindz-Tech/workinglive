        ...
        <div className="m-auto h-[80vh] sm:h-[72vh] relative px-[10px]">
          ...
          >
            <div ref={containRef} className="w-full py-2 flex flex-col">
              <div className="child h-full text-center">
                <div className="rounded-lg bg-boxFill-900 text-grey-800 inline-block p-2 leading-none sm:py-1 sm:text-xs">Today</div>
              </div>

              {messages.map(({ message, user: { id } = {} }, index) => {
                const isCurrentUser = id === user?._id;
                return (
                  <div key={index} className={`pt-[10px] ${isCurrentUser ? "text-right" : "text-left"}`}>
                    <div className={`max-w-[55%] sm:max-w-[80%] inline-block sm:text-sm p-3 ${isCurrentUser ? "bg-primary text-white rounded-b-lg rounded-tl-xl ml-auto" : "bg-secondary rounded-b-lg"}`}>{message}</div>
                    <p className="text-grey-800 mt-1 text-xs leading-none">{formatTime(new Date())}</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="absolute bottom-0 left-0 w-full h-20 sm:h-auto gap-2 flex justify-between items-center">
            <div className="relative flex-1">
              <input type="text" onKeyPress={(e) => e.key === "Enter" && sendMessage()} value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} placeholder="Send message..." className="p-3 px-5 w-full bg-boxFill-900 rounded-md outline-none" />
            </div>
